   ...
   if (lhoriz_varying_bckgrnd) then
     ...
     do i=1,nx_block
      ...
      if ( TLATD(i,j,iblock) .lt. -10.0_r8 ) then
        bckgrnd_vdc(i,j,k,iblock) = bckgrnd_vdc(i,j,k,iblock) + bckgrnd_vdc1 !bckgrnd_vdc_eq
      elseif  ( TLATD(i,j,iblock) .le. 10.0_r8 ) then
        ...
        !substitute the bckgrnd value outside the 10 degree N/S with F-scheme value.		
        !bckgrnd_vdc(i,j,k,iblock)=bckgrnd_vdc(i,j,k,iblock) + bckgrnd_vdc_eq     
      endif
...
!  huxiaokai 2020/07/09 =>
!-----------------------------------------------------------------------
!
!  Add finescale near-inertial internal wave (niiw) paramaterization (Fan,2015)
!  ,"fine_scheme" for short. 

!  It is not a good strategy that activating "fine_scheme" without
!  logical variables such as "lniw_mixing",because I dont know how to
!  realize it.(:D) In fact,"fine_scheme" is conflicted with the preexistent 
!  "niw_mixing" paramaterization (jochum,2013).Both of fine_shceme and 
!  "niw_mixing" are trying to interpret the mixing induced by near-inertial
!  waves,but "fine_scheme" is derived from Fan's(Fan,2015) pure theory work.

!  The code is transplanted from MOM4 in an unreleased version,
!  written by Fanghua Wu(Wu,2019).Since we need to calculate mixed layer depth 
!  by our own definition,here we introduced new subroutines "seawater" and "eddy"
!  The latter contains the computation of horizontal kinemitc spectrum,
!  displace spectrum and shear spectrum.They are critial to calculate A_mu and K_mu
!  (VVC and VDC in POP2) induced by near-inertial waves.
!
!-----------------------------------------------------------------------
if ( mod(nsteps_total,1*nsteps_per_day) == 0 ) then
! This program aims to calculate the value of vertical eddy viscosity 
! of momentum and eddy diffusivity of temperature and salt.  It can be
! a new diapycnal mixing scheme which may be applnx_block to Ocean General
! Circulation Model (OGCM) in the future. 
! =======================================================================
! ********************* Author: ZHANG SHANWU ****************************
! *********************** Date: 2012-5-25 *******************************
! ********************* Latest update 2012-10-12 *************************
   do j=1,ny_block
     do i=1,nx_block
       kmax = KMT(i,j,bid)
       na(i,j) = max(1,kmax-1)  ! itnu(i,j)->KMT(i,j,bid)
     end do
   end do
 ...
 do j=1,ny_block
   do i=1,nx_block
     ...
     if (KMT(i,j,bid) .ge. 0.5) then
       do k=1,km
         temp_iw(k) = TRCR(i,j,k,1)
         sal_iw(k)  = TRCR(i,j,k,2)
       end do
       call seawater(i,j,na,temp_iw,sal_iw,BV_FREQ,mld_iw,mll_iw,abs(TLATD(i,j,bid)))

       fine_mld(i,j) = mld_iw
       fine_mll(i,j) = mll_iw

       do k = 1,na(i,j)
          z_iw = c0 !0.                              ! z = 0 is at the bottom of the mixed layer     
                                                 ! and in the whole mixed layer we choose z = 0
         if (k .ge. mll_iw) then
!wfh                 z_iw = -(abs(zkp(k+1)) - mld_iw)    ! get z values under the mixed layer 
             z_iw = -(abs(zw(k+1))/100.0 - mld_iw)    ! get z values under the mixed layer 
!hxk      the unit of mld_iw is (m) zw is (cm)
!wfh             if (abs(zkp(k+1)) .gt. h_iw) then    ! if we reach the calculated depth 
!wufh                 z_iw = -(h_iw-mld_iw)                   ! set z to the value at the calculated depth,h_iw 
             if (abs(zw(k+1))/100.0 .gt. h_iw) then    ! if we reach the calculated depth 
                 z_iw = -(h_iw-mld_iw)                   ! set z to the value at the calculated depth,h_iw 
             endif
         endif
         freq_iw = BV_FREQ(k)
         call eddy(i,j,z_iw,freq_iw,A_mu(k),K_mu(k),abs(TLATD(i,j,bid)))
! --------------------------------------------------------------
       end do


       do k = 1,km-1
       akm_iw(i,j,k)=A_mu(k)!*Grd%tmask(i,j,min(k+1,nk))
       akt_iw(i,j,k)=K_mu(k)!*Grd%tmask(i,j,min(k+1,nk))
       end do
       akm_iw(i,j,km)=0.0
       akt_iw(i,j,km)=0.0

       aks_iw(i,j,:)=akt_iw(i,j,:)

! zhangsw121012
! decrease diffusivity of temperature in the upper 1500m
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!wfh         akt_iw(ix_fine,iy_fine,1:24)=1./5.*akt_iw(ix_fine,iy_fine,1:24)
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! zhangsw121012
!      endif ! for low latitude jugdement
       do k = 1,km-1
!       do j = jsc,jec
!          do i = isc,iec
!hxk          aks_iw(i,j,k)=aks_iw(i,j,k)*Grd%tmask(i,j,min(k+1,km))
!hxk          if (k.le.31) then ! decrease diffusivity of temperature in the upper 1500m
		  if (k.le.44) then ! zw(44)=1483(m) in pop2
          akt_iw(i,j,k)=akt_iw(i,j,k)*0.2!*REGION_MASK(i,j,min(k+1,km))
          else
          akt_iw(i,j,k)=akt_iw(i,j,k)!*REGION_MASK(i,j,min(k+1,km))
          endif
!         end do
!       end do
       end do
...
!    if (mytid.eq.0) write(*,*) "here6",ix_fine,iy_fine
       akm_iw(i,j,:)= c0 !0.0
       akt_iw(i,j,:)= c0 !0.0
       aks_iw(i,j,:)= c0 !0.0
     endif ! for land or sea judgement
 ...
 enddo !j
    ...
    do j = 1,ny_block
       do i = 1,nx_block
         if ( abs(TLATD(i,j,bid)).gt.10.0_r8 ) then
            
             if ( k.ge.fine_mll(i,j) ) then
               WORK1(i,j) = akm_iw(i,j,k)*10000.0 !accumulate VVC 10000for m2/s -> cm2/s
               VDC(i,j,k,1) = VDC(i,j,k,1) + akt_iw(i,j,k)*10000.0
               VDC(i,j,k,2) = VDC(i,j,k,2) + aks_iw(i,j,k)*10000.0
             endif
			 
         endif
       enddo !i
    enddo !j
		call tgrid_to_ugrid(WORK2,WORK1,bid)

		VVC(:,:,k) = VVC(:,:,k) + merge(WORK2, c0, (k < KMU(:,:,bid)))
	enddo !k
...
! !INTERFACE:
 ...
 subroutine eddy(eddy_ix,eddy_iy,eddy_z,eddy_freq,A_u,K_u,phi_iw)
 
!========================================================================
! eddy_ix       : longitude index    [unitless]         [1?ив1] 
! eddy_iy       : latitude index     [unitless]         [1?ив1]
! eddy_freq     : buoyancy frequency 
!                 at depth z         [s^-1]             [1?ив1]
! A_u           : viscosity          [m^2s^-1]          [1?ив1]
! K_u           : diffusivity        [m^2s^-1]          [1?ив1]
! 
!wfh   use precision_mod
!wfh  use pconst_mod, only: pi,lat_local,g,N_0,h_iw,b_iw,c_iw,omega_f
!hxk0715   use constants_mod, only: pi,grav,n_0,h_iw,b_iw,c_iw,omega_f,omega  ! g->grav, lat_local->grid%yt(:,:)
   implicit none
   ! 
! zhangsw120812
   integer  :: eddy_ix,eddy_iy
! zhangsw120812 
!wfh ->
!  real (r8)    :: eddy_z,eddy_freq
!  real (r8)    :: A_u,K_u
   real (r8)     :: eddy_z,eddy_freq
   real (r8)     :: A_u,K_u
!wfh <-
   ! 
! zhangsw120924
!wfh   real (r8),parameter  :: coeff=1./60.                  !  modifnx_block coefficient
   real (r8) ,parameter  :: coeff=1./60.                  !  modifnx_block coefficient
! zhangsw120924
   integer,parameter    :: beta_m=30,omega_n=50         !  for DoubleIntegral
   ! 
!wfh   real (r8),external   :: f_freq,f_h_freq
   real (r8),external   :: f_freq,f_h_freq
   ! 
!wfh   real (r8)            :: N_f,f,f_h,raddeg,omega_i,omega_c,A_l,B_u
   real (r8)            :: N_f,f,f_h,omega_i,omega_c,A_l,B_u
!  
!wfh   real (r8)            :: i_value1,i_value2,i_value3
   real (r8)            :: i_value1,i_value2,i_value3
! 
   real (r8):: raddeg 
!hxk0715 =>
   ...
   real (r8),  parameter :: grav   = 9.80
   real (r8),  parameter :: h_iw=4500.,b_iw=1300.,c_iw=1500.,omega_f=1.454e-4, N_0=0.005236
...
!hxk0713 <=
   ...
   N_f = eddy_freq
! 
!   f   = abs(omega_f*sin(lat_local(eddy_iy)*raddeg))
!   f_h = abs(omega_f*cos(lat_local(eddy_iy)*raddeg))
!hxk0713    f   = 2.0*abs(omega*sin(Grd%phiu(eddy_ix,eddy_iy)))
!hxk0713    f_h = 2.0*abs(omega*cos(Grd%phiu(eddy_ix,eddy_iy)))
	f   = abs(omega_f*sin(phi_iw*pi/180.0))
    f_h = abs(omega_f*cos(phi_iw*pi/180.0))
!   f   = abs(omega_f*sin(Grd%yt(eddy_ix,eddy_iy)/radian))
!   f_h = abs(omega_f*cos(Grd%yt(eddy_ix,eddy_iy)/radian))
! 
   omega_i = 1.01*f                          ! inertial frequency
   omega_c = 1.2*f                           ! near-inertial frequency  
   A_l = 0.001*2*pi                          ! 0.001 cpm
   B_u = 0.1*2*pi                            ! 0.1 cpm
!
   ! 
  if (N_f .gt. omega_c) then
!       call integral_spec(eddy_ix,eddy_iy,eddy_z,N_f,i_value1,i_value2,i_value3)
! ------------------------------------------------------------------------------      
!    i_value1 = ( 0.003*f*f_h/sqrt(2.)/pi*(b_iw*N_f**2/N_0/361./pi/g*               &
!               (30.*sqrt(20./19.)*log((1.+ sqrt(19./20.))/(1.-sqrt(19./20.)))-79)  &
!               +82*pi*g*N_f**2*(eddy_z+h_iw)/c_iw**2/N_0**3) ) *                   &
!               ( (log((1.2*f-f)/(1.01*f-f))-log((1.2*f+f)/(1.01*f+f)))/f     &
!               +(1./1.2/f-1./1.01/f) )
!  
!    i_value2 = 1230*sqrt(2.)*1e-4*f*f_h*g*(eddy_z+h_iw)/N_0**3/c_iw**2             &
!             *(-1./1.2/f+1./1.01/f)
!wfh    i_value1 = ( 0.003*f*f_h/sqrt(2.)/pi*(b_iw*N_f**2/N_0/361./pi/g*               &
    i_value1 = ( 0.003*f*f_h/sqrt(2.)/pi*(b_iw*(N_f*N_f)/N_0/361./pi/grav*               &
               (30.*sqrt(20./19.)*log((1.+ sqrt(19./20.))/(1.-sqrt(19./20.)))-79)  &
!wfh               +82*pi*g*N_f**2*(eddy_z+h_iw)/c_iw**2/N_0**3) ) *                   &
               +82*pi*grav*N_f*N_f*(eddy_z+h_iw)/(c_iw*c_iw)/(N_0*N_0*N_0)) ) *       &  ! g->grav
               ( (log((0.99*N_f-f)/(1.01*f-f))-log((0.99*N_f+f)/(1.01*f+f)))/f     &
               +(1./0.99/N_f-1./1.01/f) )

!wfh    i_value2 = 1230*sqrt(2.)*1e-4*f*f_h*g*(eddy_z+h_iw)/N_0**3/c_iw**2             &
    i_value2 = 1230*sqrt(2.)*1e-4*f*f_h*grav*(eddy_z+h_iw)/(N_0*N_0*N_0)/(c_iw*c_iw)  &  ! g->grav
             *(-1./0.99/N_f+1./1.01/f)
! -----------------------------------------------------------------------------
! i_value3 : shear
    call DoubleIntegral(A_l,B_u,beta_m,omega_i,omega_c,omega_n,shear_spectrum,i_value3)
! ----------------------------------------------------------------------------- 
       ! zhangsw120819
       if (i_value3 .gt. 0) then
          A_u = coeff * i_value1/sqrt(i_value3)
!wfh         K_u = coeff * N_f**2 * i_value2/sqrt(i_value3)
          K_u = coeff * N_f*N_f * i_value2/sqrt(i_value3)
       end if
       ! zhangsw120819
! zhangsw120818
  else
! zhangsw120924
       A_u = 0.0                        !  give a minimum value of viscosity
       K_u = 0.0                        !  give a minimum value of diffusivity
! zhangsw120924
! zhangsw120818
...
!------------------------------------------------------------------------------
  function shear_spectrum(beta,omega)
! ==================================================================
! Return the value of shear spectrum
! beta              : wave number             [m^-1] [1?ив1]
! omega             : frequency               [s^-1] [1?ив1] 
! shear_spectrum    : shear spectrum          
    implicit none
!wfh ->
   real (r8)          :: beta,omega
   real (r8)          :: shear_spectrum
   real (r8)          :: A_beta,B_omega
   real (r8)          :: beta_star,beta_p
!    real            :: beta,omega
!    real           :: shear_spectrum
!    real           :: A_beta,B_omega
!    real           :: beta_star,beta_p
!wfh <-
    ! 
!wfh     beta_star = 20*pi/b_iw/N_0*sqrt(N_f**2-omega**2)
!wfh     beta_p    = pi/b_iw/N_0*sqrt(N_f**2-omega**2)
     beta_star = 20*pi/b_iw/N_0*sqrt(N_f*N_f-omega*omega)
     beta_p    = pi/b_iw/N_0*sqrt(N_f*N_f-omega*omega)
     if (beta .ge. beta_p) then
!wfh        A_beta = 1.5*sqrt(N_f**2-omega**2)/sqrt(omega**2-f**2)/ &
!wfh                 (1+(beta-beta_p)/beta_star)**2.5/beta_star
        A_beta = 1.5*sqrt(N_f*N_f-omega*omega)/sqrt(omega*omega-f*f)/ &
                 (1+(beta-beta_p)/beta_star)**2.5/beta_star
     else
        A_beta=0.
     endif
!wfh    B_omega=2/pi*f/omega/sqrt(omega**2-f**2)
!wfh    shear_spectrum = f_h*N_f*(omega**2+f**2)/2/sqrt(2.)/N_0/omega/                        &
!wfh                     (sqrt(N_f**2-omega**2)*sqrt(omega**2-f**2))*                         &
!wfh                     (2*g*b_iw*N_f**2*(eddy_z+h_iw)/c_iw**2/(N_f**2-omega**2)*            &
!wfh                     beta**3+(2*N_f**2/g+2*g/c_iw**2-g*(eddy_z+h_iw)/b_iw/c_iw**2)*beta-  &
!wfh                     (N_f**2-omega**2)/2/g/b_iw**2/beta)*0.003*N_f/N_0*                   &
!wfh                     B_omega*A_beta
    B_omega=2/pi*f/omega/sqrt(omega*omega-f*f)
    shear_spectrum = f_h*N_f*(omega*omega+f*f)/2/sqrt(2.)/N_0/omega/                        &
                     (sqrt(N_f*N_f-omega*omega)*sqrt(omega*omega-f*f))*                         &
                     (2*grav*b_iw*N_f*N_f*(eddy_z+h_iw)/(c_iw*c_iw)/(N_f*N_f-omega*omega)*            &  ! g->grav
                     beta*beta*beta+(2*N_f*N_f/grav+2*grav/(c_iw*c_iw)-grav*(eddy_z+h_iw)/b_iw/(c_iw*c_iw))*beta-  & ! g->grav
                     (N_f*N_f-omega*omega)/2/grav/(b_iw*b_iw)/beta)*0.003*N_f/N_0*                   &   ! g->grav
                     B_omega*A_beta
    return
    end function shear_spectrum

    subroutine DoubleIntegral(a,b,n,c,d,m,f,int_value)
...
!wfh ->
   ...
   real(r8)                :: dx,dy
   real(r8)                :: int_value
   real(r8),dimension(n+1)     :: x
   real(r8),dimension(m+1)     :: y
   real(r8),dimension(n+1,m+1) :: ff
!    real,external       :: f
!    real                :: a,b,c,d
!    real                :: dx,dy
!    real                :: int_value
...
!wfh <-
    ...
    int_value = ff(1,1) + ff(1,m+1) + ff(n+1,1) + ff(n+1,m+1)
    do i = 2,n
         int_value = int_value + 2*(ff(i,1) + ff(i,m+1))
    end do
    do j = 2,m
         int_value = int_value + 2*(ff(1,j) + ff(n+1,j))
    end do
    do i = 2,n
        do j = 2,m
           int_value = int_value + 4*ff(i,j)
        end do
    end do
    int_value = int_value*dx*dy/4.0
    return
    end subroutine DoubleIntegral
end subroutine eddy
!huxiaokai 2019/0712 <=
...
! !IROUTINE: seawater
! !INTERFACE:
 subroutine seawater(sw_ix,sw_iy,na,sw_temp,sw_sal,sw_BV_FREQ,sw_mld,sw_mll,lat)
! ============================================================================
! To calculate the Buoyancy frequency, the mixed layer depth and the 
! mixed layer level using seawater toolbox
! sw_ix         : longitude index    [unitless]     [1?ив1]
! sw_iy         : latitude index     [unitless]     [1?ив1]
! sw_temp       : temperature        [degree C]     [1?ивkm]  LICOM: potential temperature
! sw_sal        : salinity           [unitless]     [1?ивkm]
! sw_BV_FREQ    : buoyancy frequency [s^-1]         [1?ивkmm1]
! sw_mld        : mixed layer depth  [m]            [1?ив1]
! sw_mll        : mixed layer level  [unitless]     [1?ив1]
!wfh #include <def-undef.h>
!wfh use precision_mod
!wfh use param_mod
!wfh use pconst_mod
!hxk    use constants_mod  ! wfh add
    implicit none
    integer                  :: sw_ix,sw_iy
    integer                  :: sw_mll
!wfh ->
!   real (r8)                    :: sw_lon,sw_lat
!   real (r8),dimension(km)      :: sw_temp,sw_sal
!   real (r8),dimension(kmm1)    :: sw_BV_FREQ
!   real (r8)                    :: sw_mld
    real (r8)                    :: sw_lon,sw_lat
    real (r8) ,dimension(km)      :: sw_temp,sw_sal  ! wfh km->km
    real (r8) ,dimension(km-1)    :: sw_BV_FREQ      ! wfh kmm1->km-1
    real (r8)                     :: sw_mld
!     
!   real (r8),dimension(km)      :: pot_rho,rho,p
!   real (r8)                    :: p_ref
    real (r8) ,dimension(km)      :: rho,p   ! wfh km->km
    real (r8)                    :: p_ref
!
!   For calculating sw_BV_FREQ  
!    real (r8),dimension(kmm1)                 :: Nsquare
    real (r8) ,dimension(km-1)                 :: Nsquare ! wfh kmm1->km-1
!   real (r8),allocatable,dimension      (:)  :: d_p,d_rho,d_g,p_mid,    &
    real (r8) ,allocatable,dimension      (:)  :: d_z,d_rho,d_g,p_mid,mid_g,mid_rho,   &
                                             rho_deep,rho_shallow
!   real (r8),dimension(km)                   :: grav
    real (r8) ,dimension(km)                   :: grav1   ! wfh grav->grav1 km->km
!wfh <-
    real (r8)                    ::lat
!
    integer                      :: i_local, i, j, k
    integer, dimension(1:nx_block,1:ny_block)   :: na
!   integer :: kmax
!
!    if (mytid.eq.0) write(*,*) sw_ix,sw_iy
!    if (mytid.eq.0) write(*,*) sw_temp
!    if (mytid.eq.0) write(*,*) sw_sal
!     
!hxk0715 =>
!   type (block), intent(in) :: &
!      this_block          ! block information for current block
...
!hxk <=
    sw_mll = 0
    sw_mld = 0.0
    p_ref  = 0.0               ! reference pressure for pontential density
	sw_lat = lat
	   
!wfh   do i_local = 1,itnu(sw_ix,sw_iy)
      do i_local = 1,km-1
         p(i_local)       = dep_to_pre(abs(zw(i_local)/100.0),sw_lat)  ! wfh zkt(i_local) -> zt(i_local)
         grav1(i_local) = sw_grav(sw_lat,p(i_local))  ! wfh grav->grav1
      enddo

      ! calculate Nsquare
      allocate( d_z(km-1),d_rho(km-1),d_g(km-1), p_mid(km-1),  &
                rho_deep(km-1),rho_shallow(km-1),mid_g(km-1),mid_rho(km-1) ) !itnu(i,j)->KMT(i,j)


         do i_local = 2,km   ! wfh  itnu(sw_ix,sw_iy)
            rho_deep(i_local-1)    = sw_dens0(sw_sal(i_local),sw_temp(i_local))
         enddo
		 
         do i_local = 1,km-1
            rho_shallow(i_local) = sw_dens0(sw_sal(i_local),sw_temp(i_local))
			
            d_rho(i_local)    = rho_deep(i_local) - rho_shallow(i_local)
            mid_rho(i_local)  = 0.5*(rho_deep(i_local) + rho_shallow(i_local))
            mid_g(i_local)    = 0.5*(grav1(i_local) + grav1(i_local)) 
            d_z(i_local)      = abs(zt(i_local+1)-zt(i_local)) /100.0
         enddo

         do i_local = 1, km-1
            Nsquare(i_local)=-mid_g(i_local)*d_rho(i_local)/(d_z(i_local)*mid_rho(i_local))
			sw_BV_FREQ(i_local) = sqrt(abs(Nsquare(i_local)))
         enddo
            

! determine the mixed layer depth and the mixed layer level

    do i_local = 1,km-1   ! wfh  itnu(sw_ix,sw_iy)
         sw_mll    = i_local
         sw_mld    = abs(zw(i_local)/100.0) ! wfh (zkt(i_local)) -> zt(i_local)
         if ((rho_shallow(i_local) - rho_shallow(1)) .ge. 0.125) then     ! determine the mixed layer depth by sigma criterion
! zhangsw120807          if ((pot_rho(i) - pot_rho(2)) .ge. 0.03) then
            exit
         endif
    enddo
	
      deallocate(d_z,d_rho,d_g, p_mid,  &
                rho_deep,rho_shallow,mid_g,mid_rho)

return

contains
! 
      function dep_to_pre(dpth_sw,dlat_sw)
      ! convert depth (>0) in meters to pressure in db
      ! ref: Saunders,"Practical Conversion of Pressure to Depth",
      ! J. Phys. Oceanog., April 1981.
!wfh ->
!     use precision_mod
!     use pconst_mod, only: pi
!hxk      use constants_mod, only: pi
!wfh <-
      implicit none
!wfh ->
     real (r8)  :: dep_to_pre
     real (r8)  :: dpth_sw,dlat_sw
     real (r8)  :: plat,d,c1_sw
!      real   :: dep_to_pre
!      real   :: dpth_sw,dlat_sw
!      real   :: plat,d,c1_sw
!hxk =>
	  real (r8),parameter   :: pi=3.14159265358979323846
!hxk <=
!wfh <-
      plat = abs(dlat_sw*pi/180.)
      d = sin(plat)
!wfh     c1_sw= 5.92e-3+d**2*5.25e-3
!wfh     dep_to_pre=((1-c1_sw)-sqrt(((1-c1_sw)**2)-(8.84e-6*dpth_sw)))/4.42e-6
      c1_sw= 5.92e-3+d*d*5.25e-3
      dep_to_pre=((1-c1_sw)-sqrt(((1-c1_sw)*(1-c1_sw))-(8.84e-6*dpth_sw)))/4.42e-6
      !              
      return
      end function dep_to_pre
...
!
      ...
      ! 
      !   OUTPUT:
      !   sw_dens = density  [kg/m^3]
!wfh      use precision_mod
      implicit none
!wfh ->
     real(r8) :: S,T,P
     real(r8) :: sw_dens
!     !real(r8),external :: sw_dens0,sw_seck
     real(r8) :: densP0,K
!      real :: S,T,P
!      real :: sw_dens
!      real :: densP0,K
!wfh <-
      ...
      return
        ...
        !
        ! OUTPUT:
        !   sw_dens0 = density  [kg/m^3] of salt water with properties S,T,
...
!wfh ->
       real(r8) :: S,T
       real(r8) :: sw_dens0
!       !real(r8),external :: sw_smow
       real(r8) :: T68,b0_sw,b1_sw,b2_sw,b3_sw,b4_sw,c0_sw,c1_sw,c2_sw,d0
!        real :: S,T
!        real :: sw_dens0
!        real :: T68,b0_sw,b1_sw,b2_sw,b3_sw,b4_sw,c0_sw,c1_sw,c2_sw,d0
!wfh <- 
        T68 = T * 1.00024
        b0_sw =  8.24493e-1
        b1_sw = -4.0899e-3
        ...
        sw_dens0 = sw_smow(T) + (b0_sw + (b1_sw + (b2_sw + (b3_sw + b4_sw*T68)*T68)*T68)*T68)*S &
!wfh                              + (c0_sw + (c1_sw + c2_sw*T68)*T68)*S*sqrt(S) + d0*S**2
                              + (c0_sw + (c1_sw + c2_sw*T68)*T68)*S*sqrt(S) + d0*S*S
...
!        real   :: ptmp
!wfh <-
        !real(r8),external :: sw_ptmp,sw_dens
        ...
        ! OUTPUT:
        !   sw_ptmp = Potential temperature relative to PR [degree C (ITS-90)]
!wfh        use precision_mod
        implicit none
!wfh ->
       real(r8) :: S,T,P,PR
       real(r8) :: sw_ptmp
       !real(r8),external :: sw_adtg
       real(r8) :: del_P,del_th,th,q
!        real :: S,T,P,PR
!        real :: sw_ptmp
!        real :: del_P,del_th,th,q
        real (r8) :: sqrt2
        sqrt2 = sqrt(2.)
!wfh <-
        ! theta1
        ...
        th     = th + (1 - 1./sqrt2)*(del_th - q)       ! sqrt(.)->sqrt2
        q      = (2-sqrt2)*del_th + (-2+3./sqrt2)*q     ! sqrt(.)->sqrt2
        ! theta3
        del_th = del_P*sw_adtg(S,th/1.00024,P+0.5*del_P)
        th     = th + (1 + 1./sqrt2)*(del_th - q)       ! sqrt(.)->sqrt2
        q      = (2 + sqrt2)*del_th + (-2-3./sqrt2)*q   ! sqrt(.)->sqrt2
        ...
        sw_ptmp     = (th + (del_th - 2.*q)/6.)/1.00024 
        return
        ...
        !
        ! OUTPUT:
        !  sw_adtg = adiabatic temperature gradient [degree_C/db]*0.5
!wfh        use precision_mod
        implicit none
!wfh ->
       real(r8)    :: S,T,P
       real(r8)    :: sw_adtg
       real(r8)    :: T68,a0,a1,a2,a3,b0_sw,b1_sw,c0_sw,c1_sw,c2_sw,c3,d0,d1,e0,e1,e2
!        real    :: S,T,P
!        real    :: sw_adtg
!        real    :: T68,a0,a1,a2,a3,b0_sw,b1_sw,c0_sw,c1_sw,c2_sw,c3,d0,d1,e0,e1,e2
!wfh <- 
        T68 = 1.00024 * T
        a0 =  3.5803E-5
        a1 = +8.5258E-6
        ...
        e1 = +1.8676E-14
        e2 = -2.1687E-16
        sw_adtg  =       a0 + (a1 + (a2 + a3*T68)*T68)*T68 &
                                  + (b0_sw + b1_sw*T68)*(S-35)  &
                    + ( (c0_sw + (c1_sw + (c2_sw + c3*T68)*T68)*T68) &
                                  + (d0 + d1*T68)*(S-35) )*P &
        ...
        !   T = temperature [degree C (ITS-90)]
        ! OUTPUT:
        !   sw_smow = density  [kg/m^3]
!wfh        use precision_mod
        implicit none
!wfh ->
       real(r8)   :: T
       real(r8)   :: sw_smow
       real(r8)   :: a0,a1,a2,a3,a4,a5,T68
!        real   :: T
!        real   :: sw_smow
!        real   :: a0,a1,a2,a3,a4,a5,T68
!wfh <-
        a0 = 999.842594
        a1 =   6.793952e-2
        a2 =  -9.095290e-3
        ...
        !
        ! OUTPUT:
        !   sw_seck = Secant Bulk Modulus  [bars]
!wfh        use precision_mod
        implicit none
!wfh ->
        real(r8)    :: S,T,P
        real(r8)    :: sw_seck
        real(r8)    :: T68,h0,h1,h2,h3,k0,k1,k2,e0,e1,e2,e3,e4,j0,i0,i1,i2,&
                               m0,m1,m2,f0,f1,f2,f3,g0,g1,g2
        real(r8)    :: AW,BW,KW,SR,A,B,KK
!        real    :: S,T,P
!        real    :: sw_seck
!        real    :: T68,h0,h1,h2,h3,k0,k1,k2,e0,e1,e2,e3,e4,j0,i0,i1,i2,&
!                           m0,m1,m2,f0,f1,f2,f3,g0,g1,g2
!        real    :: AW,BW,KW,SR,A,B,KK
!wfh <- 
        ...
        g1 = +1.6483E-2
        g0 = +7.944E-2
        KK = KW + (  f0 + (f1 + (f2 + f3*T68)*T68)*T68 &
              +   (g0 + (g1 + g2*T68)*T68)*SR )*S
        sw_seck = KK + (A + B*P)*P

        return
        end function sw_seck

      function sw_grav(deglat,pdb)
!wfh ->
!      use precision_mod
!      use pconst_mod, only: pi
!hxk      use constants_mod, only: pi
...
!wfh <-
      raddeg = pi/180.
      slatsq = sin(deglat*raddeg) 
      slatsq = slatsq*slatsq
      sw_grav = 9.780318*(1.0+(0.0052788+2.36e-5*slatsq)*slatsq)+2.184e-6*pdb
      return
      end function sw_grav
! 
!    
 end subroutine seawater
