...
im=3;jn=2;
nflag=2;
figure('position',[100 00 160*jn*2 120*im*2])
ha = tight_subplot(im,jn,[.06 .02],[.1 .04],[.06 .02]);
...
depstr={'0m ~ BLD','BLD ~ 1000m','100~1500m','1500~4000m','4000~bottom'};
HBLTloc=HBLTloc;
%%
for ii=1:im
    for jj=1:jn
...
%                 cb(ha(hi))=colorbar(ha(hi));colorbar off;
                ...
                if jj==1
                    hmt(ii)=m_text(445,90,titlestr{ii},'backgroundcolor', [.99 .99 .99], 'edgecolor', 'k');
                    set(hmt(ii),'position',[2.18 1.84 0])
                end
                %                 switch hi
                %                     case 1
                ...
                %                 end
                %                 if jj==1
                m_text(39.5,-75.2,depstr{jj},'fontsize',10,'fontweight','bold')
                %                 end
                title(' ')
                % xytick
                if ii<im && jj==1
        ...
        if jj==jn && ii==im
            if jvar==4||jvar==5
                ...
                set(get(cb,'title'),'string',vunit,'position',[400 0 0])
                set(cb,'ticklength',0)
            end
