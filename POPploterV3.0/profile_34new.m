...
titlestr{1}=[casealias{2},' - ',casealias{1}];
titlestr{2}=[casealias{3},' - ',casealias{1}];
titlestr{3}=[casealias{4},' - ',casealias{1}];
% titlestr{4}=[casealias{5},' - ',casealias{1}];
...
figure('position',[100 60 225*jn*1.2 190*im*1.2])
ha = tight_subplot(im,jn,[.06 .01],[.095 .065],[.05 .03]);
for ii=1:im
    for jj=1:jn
        hi=hi+1
        %         nk=floor((hi-1)/4)+1;
...
%                 tmask(abs(interpLAT)>50.5)=nan;
            ...
            case 2
             ...
             case 3
              ...
              case 4
                timestr='_avg_296_300';
                %                 vn1str1=[casename{jj},'_'];vn1str2=[variname{j},timestr];
                vn1str1=[casename{5},'_'];vn1str2=[variname{jvar},timestr];
                switch jvar
                    case 4
                        vn2str1='tempphc';vn2str2='';
                    case 5
        ...
        switch pictype
            case 'profile'
                %         profile[zbt_interpLAT_mesh_1,tpdepticklabel]=unequalcmap(zbt_interpLAT_mesh,tpdeplev);
                ...
                ytxt=min(Y)-0.1*(max(Y)-min(Y));
                
                
                %title
                
                if jj==1
                    text(-85,ytxt,titlestr{ii},'interpreter','none','fontsize',8,'fontweight','bold',...
                ...
                if hi==1
                    text(xtxt,ytxt,seaname{5},'fontsize',8,'fontweight','bold',...
                    'backgroundcolor', [.99 .99 .99], 'edgecolor', 'k')
                else
                    text(xtxt,ytxt,seaname{jj-1},'fontsize',8,'fontweight','bold',...
                    'backgroundcolor', [.99 .99 .99], 'edgecolor', 'k')
                end
                end
