...
im=2;jn=2;
nflag=2;
figure('position',[100 60 235*jn*2 165*im*2])
ha = tight_subplot(im,jn,[.02 .02],[.08 .03],[.035 .015]);
...
%                 vn2str1=[casename{2},'_'];vn2str2=[variname{jvar},timestr];
        ...
        switch pictype
            case 'map'
                ...
                % clevl=clevl';
                clevl=cmin:dv:cmax;
                eval(['VV=',vn1,'(:,:,deploc(hi)+1:deploc(hi+1))-',vn2,'(:,:,deploc(hi)+1:deploc(hi+1));'])
                %eval(['Vq=',vn1,'(:,:,deploc(ii))-',vn2,'(:,:,deploc(ii));'])
                VV=VV.*dz_3d(:,:,deploc(hi)+1:deploc(hi+1));
                for iin=1:360
                    for jjn=1:341
                        tV=VV(iin,jjn,:);
                        Vq(iin,jjn)=nansum(tV)/sum(squeeze(~isnan(tV)).*dz(deploc(hi)+1:deploc(hi+1)));
...
%                 m_pcolor(interpLONG,interpLAT,Vq);
                ...
                if hi==1
                    m_text(485,85,titlestr{hi},'backgroundcolor', [.99 .99 .99], 'edgecolor', 'k')
                end
                %                 switch hi
                %                     case 1
                ...
                %                 end
                %                 if jj==1
                m_text(39.5,-75.2,depstr{hi},'fontsize',8,'fontweight','bold')
                %                 end
                title(' ')
                % xytick
                if ii<im && jj==1
        ...
        if jj==jn && ii==im
            if jvar==4||jvar==5
                ...
                set(get(cb,'title'),'string',vunit,'position',[600 0 0])
                set(cb,'ticklength',0)
            end
