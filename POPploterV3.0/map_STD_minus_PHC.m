...
im=2;jn=2;
nflag=2;
figure('position',[100 00 220*jn*2 185*im*2])
ha = tight_subplot(im,jn,[.15 .02],[.1 .03],[.05 .02]);
...
%                 cb(ha(hi))=colorbar(ha(hi));colorbar off;
                ...
                if jj==1
                    m_text(475,88,titlestr{ii},'backgroundcolor', [.99 .99 .99], 'edgecolor', 'k')
                end
                %                 switch hi
                %                     case 1
        ...
        if jj==jn %&& ii==im
            if jvar==4||jvar==5
                cb(ii)=colorbar('horiz',...
                    'position',[0.1 -0.5*ii+1.028 0.8 0.02]);
                colormap(cb(ii),cmap)
                if ii==1
                    caxis([-3.25 3.25])
                elseif ii==2
                ...
                %                 pause
                    ...
                    set(cb(ii),'xtick',cmin:dv:cmax)%,'xticklabel',sprintf('\n%1.1f',cmin:dv:cmax));


                
                set(get(cb(ii),'title'),'string',vunit,'position',[555 0 0])
                set(cb(ii),'ticklength',0)
            end
